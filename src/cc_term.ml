...
and binding = string option * datum
[@@deriving show]



(* Modify the variable 'leaves' of a term. *)
let map_term (f : binding list -> string -> term) : term -> term =
  let rec aux bvs trm =
    match trm with
...
let capture_vars (xs : binding list) (trm : term) =
...
let bind_vars (bb : binder) (xs : binding list) (trm : term) =
  let rec aux ys xs =
    match xs with
    | [] -> ys
    | (str_opt, dat) :: xs' ->
...
(* -------- Pretty printing --------- *)
let rec string_of_term_aux (bvs : binding list) (t : term) =
  match t with
  | Univ Kind -> "KIND"
  | Univ Type -> "TYPE"
  ...
  | Bound i ->
    ...
    | Some x -> Printf.sprintf "(b%d ⇾ %s)" i (string_of_binding bvs x)
  ...
  | Bind(bb, (x, d), t') ->
      Printf.sprintf "%s %s. %s"
        (string_of_binder bb) (string_of_binding bvs (x,d))
...
and string_of_binding bvs (str_opt, datum) =
